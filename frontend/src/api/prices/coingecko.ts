...
export const isCoingeckoToken = (x: any): x is CoingeckoTokenName =>
  validCoingeckoTokens.includes(x);

const COINGECKO_API_URL = "https://api.coingecko.com/api/v3/simple/price";

export const getRelativePriceFromCoingecko = async (
  numeratorTokenName: CoingeckoTokenName,
  denominatorTokenName: CoingeckoTokenName
): Promise<number> => {
