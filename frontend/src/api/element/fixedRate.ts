...
export const getFixedRate = async (tokenName: string, trancheAddress: string, elementAddresses: ElementAddresses, signerOrProvider: Signer | ethers.providers.Provider): Promise<number> => {
    ...
    const reserves = await getReserves(ptPool.address, balancerAddress, signerOrProvider);
