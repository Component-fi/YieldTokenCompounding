...
export const executeYieldTokenCompounding = async (
  userData: YTCInput,
  expectedYieldTokens: number,
  expectedBaseTokensSpent: number,
  ...
  const expectedYieldTokensAbsolute = ethers.utils.parseUnits(
    expectedYieldTokens.toString(),
    yieldCalculationParameters.yieldTokenDecimals
  );
  ...
  try {
    ...
    expectedBaseTokensSpentAbsolute = ethers.utils.parseUnits(
      expectedBaseTokensSpent.toString(),
      yieldCalculationParameters.baseTokenDecimals
    );
  } catch {
    expectedBaseTokensSpentAbsolute = ethers.utils.parseUnits(
      expectedBaseTokensSpent.toFixed(6),
      yieldCalculationParameters.baseTokenDecimals
    );
  }
  ...
  const maximumSlippageYTokens =
    expectedYieldTokens * (slippageTolerancePercentage / 2 / 100);

  // The maximum number of base tokens that can be lost due to slippage
  // This will be half the maximum slippage
  const maximumSlippageBaseTokens =
    expectedBaseTokensSpent * (slippageTolerancePercentage / 2 / 100);

  let maximumSlippageYTokensAbsolute;
  try {
    // If there are too many decimals parsing the units will fail, thus we cut it to 6 decimals
    maximumSlippageYTokensAbsolute = ethers.utils.parseUnits(
      maximumSlippageYTokens.toString(),
      yieldCalculationParameters.yieldTokenDecimals
    );
  } catch {
    maximumSlippageYTokensAbsolute = ethers.utils.parseUnits(
      maximumSlippageYTokens.toFixed(6),
      yieldCalculationParameters.yieldTokenDecimals
    );
  }
  ...
  try {
    // If there are too many decimals parsing the units will fail, thus we cut it to 6 decimals
    maximumSlippageBaseTokensAbsolute = ethers.utils.parseUnits(
      maximumSlippageBaseTokens.toString(),
      yieldCalculationParameters.baseTokenDecimals
    );
  } catch {
    maximumSlippageBaseTokensAbsolute = ethers.utils.parseUnits(
      maximumSlippageBaseTokens.toFixed(6),
      yieldCalculationParameters.baseTokenDecimals
    );
  }

  // The minimum number of yTokens that should be received
  const minimumYieldAbsolute = expectedYieldTokensAbsolute.sub(
    maximumSlippageYTokensAbsolute
  );
  const maximumSpentAbsolute = expectedBaseTokensSpentAbsolute.add(
  ...
  const transaction = await ytc.compound(
    userData.numberOfCompounds,
    userData.trancheAddress,
    yieldCalculationParameters.balancerPoolId,
    yieldCalculationParameters.baseTokenAmountAbsolute,
    minimumYieldAbsolute,
    maximumSpentAbsolute
  );
