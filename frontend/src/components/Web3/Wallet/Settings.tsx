...
export const WalletSettings: React.FC<Props> = (props) => {
  ...
  const handleSlippageInputChange: React.ChangeEventHandler<
    HTMLInputElement
  > = (event) => {
      ...
      <PopoverContent>
        ...
        <PopoverBody>
          <div className="flex flex-col">
            <div id="slippage" className="text-sm text-left">
              Slippage Tolerance
            </div>
            <div className="flex flex-row justify-center gap-4 items-center">
              <ButtonGroup>
                {[0.1, 0.5, 1].map((percentage) => (
                  <button
                    ...
                    className={`hover:bg-gray-50 px-2 py-1 rounded-2xl ${
                      percentage === slippageTolerance && "text-indigo-400"
                    }`}
                    key={percentage}
                  >
                    {percentage}%
              ...
              <div className="shadow-inner flex flex-row items-center p-2 w-16 rounded-xl">
                <input
                  ...
                  className="min-w-0 text-indigo-400"
                ></input>
                <div>%</div>
              </div>
            </div>
