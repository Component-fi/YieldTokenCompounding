...
export const useYieldTokenPrice = (baseTokenName: string | undefined, trancheAddress: string | undefined) => {
    const [isLoading, setIsLoading] = useState<boolean>(false);
    const [price, setPrice] = useState<number>(0);
    ...
    useEffect(() => {
        if (baseTokenName && trancheAddress && provider){
            setIsLoading(true);
            getYTCSpotPrice(baseTokenName, trancheAddress, elementAddresses, provider).then((price) => {
                setPrice(price);
            }).catch((error) => {
                console.error(error);
            }).finally(() => {
