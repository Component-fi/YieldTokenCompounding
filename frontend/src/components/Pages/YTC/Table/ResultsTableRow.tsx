...
export const ResultsTableRow: React.FC<ResultsTableRowInterface> = (props) => {
  ...
  return (
    ...
    >
      <Td isNumeric>{output.inputs.numberOfCompounds}</Td>
      <Td isNumeric>{shortenNumber(output.receivedTokens.yt.amount)}</Td>
      <Td isNumeric>
        {shortenNumber(output.spentTokens.baseTokens.amount)}
        <br />
        (
        <BaseTokenPriceTag
          amount={output.spentTokens.baseTokens.amount}
      ...
      >
        ...
        <Tooltip
          label={`Return on investment over the term annualized. ${
            output.gain && shortenNumber(output.gain.roi)
          }% x ${
            output.gain && shortenNumber(output.gain.apr / output.gain.roi)
          }`}
        >
