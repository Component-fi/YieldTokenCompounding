...
export const ExposureBar: React.FC<ExposureBarProps> = (props) => {
  ...
  if (trancheRate.accruedValue) {
    minimumRedemptionBaseTokens =
      minimumYTokensReceived * trancheRate.accruedValue;
    variableExposureTokens = tokensSpent - minimumRedemptionBaseTokens;
    minimumRedemptionPercentage =
      (minimumRedemptionBaseTokens / tokensSpent) * 100;
    variableExposurePercentage = (variableExposureTokens / tokensSpent) * 100;
  }

  if (
    minimumRedemptionBaseTokens &&
    variableExposureTokens &&
    minimumRedemptionPercentage &&
    minimumRedemptionBaseTokens
  ) {
    return (
        ...
        <Flex flexDir="row" w="full" h={8} align="center">
          ...
          <Tooltip label={copy.tooltips.minimum_redemption}>
            <Flex
              ...
              p={1}
            >
              <InfoOutlineIcon color="grey.400" w={2.5} h={2.5} />
            </Flex>
          </Tooltip>
          <Flex bgColor="black" h="full" w="0.5"></Flex>
          <Tooltip label={copy.tooltips.variable_exposure}>
            <Flex
              bgColor="component.orange"
              w={`${variableExposurePercentage}%`}
              justify="center"
              align="center"
              h="70%"
              _hover={{
