  ...
  return (
    <DetailPane mx={{ base: 0, sm: 8 }}>
      ...
      <DetailItem
        ...
        value={
          netGain ? (
            ...
            >
              <Text>{shortenNumber(netGain)}</Text>
              (
              <BaseTokenPriceTag
                amount={netGain}
      ...
      <DetailItem
        ...
        value={
          roi ? (
            <Text color={roi > 0 ? "green.600" : "red.500"}>
              {shortenNumber(roi)}%
            </Text>
          ) : (
            "?"
      ...
      />
      <DetailItem
        name={<TextAndTooltip text="APR" tooltip={copy.tooltips.apr} />}
        value={
          apr ? (
            <Text color={apr > 0 ? "green.600" : "red.500"}>
              {shortenNumber(apr)}%
            </Text>
          ) : (
            "?"
      ...
      <Collapse in={show}>
        <DetailItem
          ...
          value={
            <Flex flexDir="row" gridGap={1}>
              <Text>{shortenNumber(minimumReceived)}</Text>
              (
              <YTPriceTag
        ...
        <DetailItem
          ...
          value={
            expectedReturn ? (
              <Flex flexDir="row" gridGap={1}>
                <Text>{shortenNumber(expectedReturn)}</Text>
                (
                <BaseTokenPriceTag
        ...
        <DetailItem
          ...
          value={
            <Flex flexDir="row" gridGap={1}>
              <Text>{shortenNumber(estimatedGas)} ETH</Text>
              (<BaseTokenPriceTag amount={estimatedGas} baseTokenName={"eth"} />
              )
            </Flex>
          }
        />
...
const TextAndTooltip: React.FC<TextAndTooltipProps> = (props) => {
  ...
  return (
    <Flex flexDir="row" alignItems="center" gridGap={1}>
      <Text>{text}</Text>
      {tooltip && <InfoTooltip w={2} h={2} label={tooltip} />}
    </Flex>
  );
};
