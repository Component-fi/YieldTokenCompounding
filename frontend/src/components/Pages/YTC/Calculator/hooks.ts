...
export const useSetFormikValueToQueryParam = (
  queryName: string,
  fieldName: string,
...
export const useSetQueryParamToFormikValue = (
  queryName: string,
...
export const useSimulate = () => {
  ...
  const { variable } = useRecoilValue(
    trancheSelector(values.trancheAddress || "")
  );
...
export const useTokenName = (tokenAddress: string | undefined) => {
  ...
  const getTokenName = useMemo((): string | undefined => {
    if (!tokenAddress) {
      return undefined;
    }
